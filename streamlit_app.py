...
df_sub_categories = df[df["Category"] == selected_category]
sub_categories = df_sub_categories["Sub_Category"].unique().tolist()
...
sales_by_month = df_sales_filterd.filter(items=['Sales']).groupby(pd.Grouper(freq='M')).sum()
st.line_chart(sales_by_month, y="Sales")

st.write("### (4) show three metrics (https://docs.streamlit.io/library/api-reference/data/st.metric) for the selected items in (2): total sales, total profit, and overall profit margin (%)")
total_sales = "${:,.2f}".format(df_sales_filterd["Sales"].sum())
total_profit = "${:,.2f}".format(df_sales_filterd["Profit"].sum())
